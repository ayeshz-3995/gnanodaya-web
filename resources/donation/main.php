<?php
    $st->pageheading('asset/img/pageheading/donationheading.jpg','ආධාර');
    ?>
<section id="sponcers">
    <div class="container">

        <div class="col-md-12 pt-5 text-center">
            <img src="asset/img/donation/images.jpg" class="imageupper">
        </div>
        <div class="row masssege">

            <p class="title pt-3">පින් කැමති අයට පින්බර අවස්ථාවක්</p>
            <hr>
            <i class="fas fa-dharmachakra"></i>
            <p class="title">තෙරුවන් සරණයි!</p>
            <p class="introduction">ඒ භාග්‍යවත් වූ අර්හත් වූ
                සම්මා සම්බුදුරජාණන් වහන්සේගේ
                ශ‍්‍රීමත් සර්වඥ ධාතූන් වහන්සේලා ද
                සම්බුදු රජුන්ගේ මහා ශ‍්‍රාවකයන් වහන්සේ නමක් වූ,
                අපමණ තේජෝ බල ඇති, අපමණ අනුකම්පා ඇති,
                ආර්ය වූ අපගේ අංගුලිමාල මහරහතන් වහන්සේගේ
                ශ‍්‍රීමත් ධාතූන් වහන්සේලා ද
                වඩාහිඳුවීමෙන් සුපූජනීය වූ,
                මහා තෙද, මහා බල ආශිර්වාදය පැතිරුණ
                ඒ භාග්‍යවත් වූ අර්හත් වූ
                සම්මා සම්බුදුරජාණන් වහන්සේගේ
                ශ‍්‍රීමත් සර්වඥ ධාතූන් වහන්සේලා ද
                සම්බුදු රජුන්ගේ මහා ශ‍්‍රාවකයන් වහන්සේ නමක් වූ,
                අපමණ තේජෝ බල ඇති, අපමණ අනුකම්පා ඇති,
                ආර්ය වූ අපගේ අංගුලිමාල මහරහතන් වහන්සේගේ
                ශ‍්‍රීමත් ධාතූන් වහන්සේලා ද
                වඩාහිඳුවීමෙන් සුපූජනීය වූ,
                මහා තෙද, මහා බල ආශිර්වාදය පැතිරුණ</p>
        </div>

        <div class="row pt-5 mt-5">
            <div class="col-md-8 p-2">
                <img src="asset/img/donation/2.jpg" class="d-img" alt="...">
            </div>
            <div class="col-md-4 p-2">
                <img src="asset/img/donation/3.jpg" class="d-img mb-2">
                <img src="asset/img/donation/4.jpg" class="d-img" alt="...">
            </div>
        </div>
    </div>

</section>


<section id="donationpage">

    <div class="col-md-12 background p-5">

        <p class="donate pt-3"> ඥානෝදය විහාරස්ථානයේ ඉදිකිරීම් සදහා පින්වත් ඔබගේ ආධාර පහත ගිණුමට බැර කළ හැකිය</p>

        <div class="col-md-12 pt-3 account">
            <p class="bank"> ඥානෝදය සංවර්ධන අරමුදල</p>
            <p class="bank">ගිණුම් අංකය</p>
            <p class="number">

                <span class="color">000000000</span>
            </p>
            <p class="bank">සම්පත් බැංකුව, හෙට්ටිපොළ ශාඛාව.</p>
        </div>
    </div>
</section>


<section id="spon">
    <div class="container">
        <div class="row text-center">
            <p class="name p-5"> අප හා එක් වු ඔබ සැමට ස්තුතියි </p>

            <div class="col-md-3">
                <img src="asset/img/donation/5.png" class="logo m-2">
            </div>

            <div class="col-md-3">
                <img src="asset/img/donation/5.png " class="logo  m-2">
            </div>

            <div class="col-md-3">
                <img src="asset/img/donation/5.png " class="logo m-2">
            </div>

            <div class="col-md-3">
                <img src="asset/img/donation/5.png " class="logo m-2">
            </div>
        </div>

    </div>
</section>